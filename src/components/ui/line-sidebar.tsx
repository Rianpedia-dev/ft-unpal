...
export const LineSidebar = ({
  ...
  const handlePointerMove = useCallback(
    ...
    [enableTracking, falloff, proximityRadius, startLoop]
  );
  ...
  return (
      ...
      style={
        {
          ...
          '--marker-length': showMarker ? `${markerLength}px` : '0px',
          '--marker-gap': showMarker ? `${markerGap}px` : '0px',
          '--tick-scale': tickScale,
          '--max-shift': enableTracking ? `${maxShift}px` : '0px',
          '--item-gap': `${itemGap}px`,
    ...
    >
      <ul
        ...
        className="m-0 flex list-none flex-col py-1 [gap:var(--item-gap)]"
      >
        {items.map((label, index) => (
          <li
            ...
            className={`relative ${onItemClick || enableTracking ? 'cursor-pointer' : 'cursor-default'} ${showMarker ? "before:absolute before:-inset-x-12 before:-inset-y-[6px] before:content-['']" : ""} ${tickClass}`}
          >
            {showMarker && (
              <span
                aria-hidden="true"
            ...
            <span className="relative inline-flex items-baseline leading-[1.3] [color:color-mix(in_srgb,var(--accent-color)_calc(var(--effect,0)*100%),var(--text-color))] [font-size:var(--font-size)] [transform:translateX(calc(var(--effect,0)*var(--max-shift)))] font-medium">
              {showIndex && (
                <span className="mr-[0.6rem] font-mono text-[0.85em] [opacity:calc(0.7+var(--effect,0)*0.3)] font-bold text-slate-400">
